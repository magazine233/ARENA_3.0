"""Build TALK_10MIN.pptx — the ~10-minute VOICE cut (parallel to build_deck_voice.py).

The 8-minute voice deck plus TWO additive slides: (3) HatCat, the programme the
hypothesis comes from (and the bookend for the close); (9) the geometry near-miss
(anisotropy correction) — the strongest non-probe evidence for the mechanism and the
second "we nearly fooled ourselves" rigor beat. Same numbers, same claims, same voice.
`TALK_10MIN.md` is the readable script. The 8-minute decks are untouched.

Layout helpers mirror build_deck.py / build_deck_voice.py (self-contained so the cuts
stay independent). Run from project/capstone/ (after render_slide_assets.py):
    python paper/slides/build_deck_10min.py
"""
...
OUT = Path("paper/slides/TALK_10MIN.pptx")
...
notes(s, "Safety monitoring wants a detector for each behaviour we care about — a lens for "
         ...
         "here's the idea: you lean on that graph. You train each lens against its relatives. "
         "And where two concepts are joined by lots of independent paths, you pin the boundary "
         "between them without ever showing the probe that boundary directly. It's the "
         "appealing kind of idea. Structure is free. Data isn't. (0:55)")

# ---- 3. HatCat (NEW) ---------------------------------------------------------
s = slide("The bigger picture — HatCat", "1:15")
bullets(s, [
    ("HatCat — an interpretability programme building a navigable map of the concepts a model represents.",
     {"size": 20, "bold": True}),
    ("", {"size": 8}),
    ("Not one probe — an atlas of per-concept “lenses,” wired together by a relational graph.", {"size": 18}),
    ("The bet: know how concepts relate →", {"size": 18}),
    ("    · train better lenses (each against its neighbours)", {"size": 17, "level": 1}),
    ("    · cover the space more cheaply — the graph says where to look,", {"size": 17, "level": 1}),
    ("      the connections reach concepts you have no direct data for.", {"size": 17, "level": 1}),
    ("", {"size": 8}),
    ("Our capstone tests the sharpest, most falsifiable slice of that bet:", {"size": 18}),
    ("Does the graph actually help — and does connectivity scale?",
     {"size": 21, "bold": True, "color": ACCENT}),
], left=0.7, top=1.4, width=12.0, height=5.6)
notes(s, "A word on where this comes from, because it's the whole reason the question matters. "
         "It isn't ours. It sits inside a programme called HatCat — an attempt to build "
         "something like a map of the concepts a model carries in its head. Not one detector "
         "bolted on the side, but a whole atlas of them: a lens per concept, and a graph wiring "
         "the related ones together. The bet is simple, and ambitious. Know how the concepts "
         "relate, and you can build better detectors — you train each one against its "
         "neighbours. And you cover the space more cheaply. The graph tells you where to look. "
         "The connections let you reach concepts you've got no direct data for. That last part — "
         "reaching a concept through its connections — is the boldest claim. It's the one we set "
         "out to test, in its sharpest, most falsifiable form. Does the graph actually earn its "
         "keep? And does the connectivity scale? (1:15)")

# ---- 4. Setup ----------------------------------------------------------------
s = slide("Setup in 30 seconds", "0:30")
...
notes(s, "Thirty seconds of setup, all of it the ARENA toolkit. Mean-pooled residual stream, "
         ...
         "and you've measured nothing. (0:30)")

# ---- 5. Act 1: the honest null -----------------------------------------------
...
notes(s, "Iteration one looked positive. It was wrong. We pulled it apart and found noise, not "
         ...
         "each concept instead of teaching it the boundary. So we rebuilt it properly. "
         "Iteration two: a hundred and five matched pairs, powered, the decision rule locked "
         "before we looked. The answer came back a clean null — connectivity does not predict "
         "how much a probe improves. Because we'd pre-registered, there was nowhere to hide it "
         "and no reason to. Hold onto that null. A few slides from now it stops being a "
         "disappointment and becomes the point. (1:00)")

# ---- 6. Act 2: the reframe ---------------------------------------------------
s = slide("Act 2 — the reframe that worked: relations as hard negatives", "1:10")
table(s, [
...
notes(s, "One picture explains all of it. A probe's boundary is set by whichever negative sits "
         ...
         "needs is independent constraints. (0:35)")

# ---- 9. Geometry near-miss (NEW) ---------------------------------------------
s = slide("A second time we nearly fooled ourselves", "0:55")
image(s, ASSETS / "slide9_anisotropy.png", 1.4, 1.55, width=10.5)
notes(s, "Quick aside, because it's the kind of thing that should make you trust the rest. When "
         "we looked at the raw geometry — just the cosine between concept clusters — it said "
         "everything overlapped. Adjacent concepts at point-nine-seven, non-adjacent at "
         "point-nine-six. For a moment it looked like the graph wasn't even in the model. But "
         "that flatness is an artifact: transformer activations get squashed into a narrow cone, "
         "so everything looks similar to everything. Subtract that out, and the structure snaps "
         "back — related concepts pull apart, and far-away ones, dogs and weather, fall right "
         "outside the cluster. Same story the probes told: the local structure is real, the "
         "long-range structure isn't. That's twice now the raw data nearly lied to us, and twice "
         "a control caught it. That's the job. (0:55)")

# ---- 10. So what -------------------------------------------------------------
s = slide("So what — you seal exactly the faces you train against", "0:55")
table(s, [
...
notes(s, "This pays off in something you can actually build. A flat fifteen-way probe has no "
         ...
         "expensive money only on the gap. (0:55)")

# ---- 11. Close ---------------------------------------------------------------
s = slide("What this taught me", "0:50")
bullets(s, [
    ("Twice the data nearly fooled us — both times the discipline caught it.", {"size": 19, "italic": True, "color": GREY}),
    ("", {"size": 6}),
    ("Pre-register before powered runs — saved us twice", {"size": 21, "bold": True}),
    ("Adversarially verify your positives — killed a false one", {"size": 21, "bold": True}),
    ("A bounded answer + a mechanism beats an unbounded yes", {"size": 21, "bold": True}),
    ("", {"size": 8}),
    ("The relationships weren't a classifier. They're a map — of where to look next.",
     {"size": 19, "bold": True, "color": ACCENT}),
    ("(which is exactly what HatCat set out to build)", {"size": 15, "italic": True, "color": GREY}),
], top=1.5)
notes(s, "What I'll take from this. Twice the data nearly fooled us — iteration one's noise, "
         "then the geometry artifact — and both times the discipline caught it: pre-register, "
         "try to kill your own positives, bound what you claim. That's the only reason I can "
         "stand here and trust these numbers. And the relationships — HatCat's whole bet? They "
         "turned out not to be a way of telling concepts apart at all. They're a map — of where "
         "to look next. Which is exactly what HatCat set out to build. Thanks. Happy to take "
         "questions. (0:50)")
